...
namespace QuickLib.Utility
{
    ...
    public class ImageHelper
    {
        ...
        /// <returns></returns>
        public static bool SaveUrlToImage(string locationPath, string imgUrl)
        {
            System.Net.WebResponse webres = null;
            try
            {
                ...
                {
                    Image.FromStream(stream).Save(locationPath);
                }
        ...
        /// <param name="location">水印位置（传送正确的代码）</param>
        /// <returns></returns>
        public static string ImageWatermarkByTemplet(string path, string waterpath, string location)
        {
            string kz_name = Path.GetExtension(path);
            ...
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg" || kz_name == ".png")
            {
                ...
                waterimg.Dispose();
                g.Dispose();
                newpath = Path.GetDirectoryName(waterpath) + "\\Water_" + Path.GetFileName(waterpath);
                img.Save(newpath);
                img.Dispose();
            }
            return newpath;
        ...
        /// <param name="path">需要加载水印的图片路径（绝对路径）</param>
        /// <param name="waterpath">水印图片（绝对路径）</param>
        /// <param name="location">水印位置（指定位置）</param>
        /// <returns></returns>
        public static string ImageWatermarkByTemplet(string path, string waterpath, List<int> location)
        {
            string kz_name = Path.GetExtension(path);
            string newpath = string.Empty;
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg" || kz_name == ".png")
            {
                ...
                Graphics g = Graphics.FromImage(img);
                //ArrayList loca = GetLocation(location, img, waterimg);
                g.DrawImage(waterimg, new Rectangle(location[0], location[1], waterimg.Width, waterimg.Height));
                waterimg.Dispose();
                g.Dispose();
                newpath = Path.GetDirectoryName(waterpath) + "\\Water_" + Path.GetFileName(waterpath);
                img.Save(newpath);
